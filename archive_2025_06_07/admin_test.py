#!/usr/bin/env python3
"""
Hedge Intelligence Admin Panel Test Suite
Date: 2025-06-05 13:26:51 UTC
Author: thorrobber22
...
def test_load_ipo_calendar(config):
    ...
    with patch('admin.CACHE_DIR', config.CACHE_DIR):
...
def test_document_coverage(config):
    ...
    with patch('admin.DOCUMENTS_DIR', config.DOCUMENTS_DIR):
        with patch('admin.SUPPORTED_DOCUMENTS', config.SUPPORTED_DOCUMENTS):
...
def test_uploaded_documents(config):
    ...
    with patch('admin.DOCUMENTS_DIR', config.DOCUMENTS_DIR):
        with patch('admin.detect_document_type') as mock_detect:
...
def test_streamlit_components():
    ...
    try:
        ...
        from admin import st
        assert hasattr(st, 'title'), "Streamlit should have title method"
        assert hasattr(st, 'columns'), "Streamlit should have columns method"
...
def run_integration_test():
    ...
    print_info("To fully test the admin panel:")
