#!/usr/bin/env python3
"""
Create admin with real data only and dropdown-based uploads
Date: 2025-06-06 13:29:40 UTC
Author: thorrobber22
"""

real_admin = '''import streamlit as st
import os
                ...
                if parts:
                    ...
                    for doc_type in REQUIRED_DOCS:
                        if doc_type.lower().replace("-", "") in file_lower:
                            ...
                            # Update last updated
                            if not status[ticker]['last_updated'] or \
                               status[ticker]['documents'][doc_type]['date'] > status[ticker]['last_updated']:
...
with open("admin_real_data.py", "w", encoding="utf-8") as f:
    f.write(real_admin)

print("✓ Created admin_real_data.py")
print("\nFeatures:")
print("- NO dummy/fallback data - real scraped IPOs only")
print("- Easy dropdown ticker selection for uploads")
print("- Shows which tickers need documents")
print("- Clear status indicators")
print("- Auto-refreshes IPO data every 30 minutes")
print("\nRun: streamlit run admin_real_data.py")
