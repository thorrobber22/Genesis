...
def render_chat():
    ...
    with st.sidebar:
        ...
        for session in recent_sessions:
            if st.button(
                ...
                type="secondary" if session['id'] != st.session_state.get('current_chat_id') else "secondary"
            ):
    ...
    if hasattr(st.session_state, 'chat_context') and st.session_state.chat_context:
        st.secondary(f"Analyzing: {st.session_state.chat_context}")
                ...
                with col3:
                    if 'sources' in metadata and metadata['sources']:
                        ...
                        if st.button("Download", key=f"dl_{message['timestamp']}"):
                            st.secondary("Download feature coming soon")
...
def show_sources(sources):
    ...
    if not sources:
        st.secondary("No sources available")
        return
