...
def render_chat_bar():
    """Render fixed chat bar - ENHANCED VISIBILITY"""
    # Create a prominent chat container
    st.markdown("""
    <div style='
        position: fixed;
        bottom: 0;
    ...
    # Add spacing so content doesn't overlap
    ...
    if user_input:
            ...
            print(f"[CHAT DEBUG] Messages in state: {len(st.session_state.chat_messages)}")
            
            # Get response
            try:
                from services.ai_service import get_ai_response
                print("[CHAT DEBUG] Importing AI service...")
                ...
                print("[CHAT DEBUG] Calling get_ai_response...")
                response = get_ai_response(user_input, context)
                print(f"[CHAT DEBUG] Got response: {response[:50]}...")
                
                st.session_state.chat_messages.append({"role": "assistant", "content": response})
                print(f"[CHAT DEBUG] Total messages: {len(st.session_state.chat_messages)}")
            except Exception as e:
                print(f"[CHAT DEBUG] ERROR: {e}")
                import traceback
                traceback.print_exc()
