"""
analyze_genesis_fixed.py - Complete analysis of Genesis repository (FIXED)
...
# Current time for 2-day cutoff
CURRENT_TIME = datetime(2025, 6, 12, 0, 47, 11)
CUTOFF_DATE = CURRENT_TIME - timedelta(days=2)

print("GENESIS REPOSITORY ANALYSIS")
print("="*80)
print(f"Date: {CURRENT_TIME.strftime('%Y-%m-%d %H:%M:%S')} UTC")
print(f"Repository: thorrobber22/Genesis")
print(f"Cutoff Date: {CUTOFF_DATE.strftime('%Y-%m-%d')}")
print("="*80)

# Based on roadmap requirements, these are the ONLY files we need
REQUIRED_FILES = {
    # Core Application
    "app.py": "Main Streamlit application with terminal UI",
    "requirements.txt": "Python dependencies",
    ".env": "Environment variables (API keys)",
    ".gitignore": "Git ignore rules",
    "README.md": "Project documentation",
    
    # Services (Phase 1 - All Complete)
    "services/ai_service.py": "AI chat integration with OpenAI",
    "services/data_service.py": "Data management and caching",
    "services/sec_service.py": "SEC EDGAR API integration",
    
    # Components (UI)
    "components/ipo_calendar.py": "IPO calendar view (Phase 1)",
    "components/companies.py": "Companies detailed view (Phase 1)",
    "components/metrics.py": "Metrics dashboard (Phase 1)",
    "components/chat.py": "AI chat interface (Phase 1)",
    "components/watchlist.py": "Watchlist management (Phase 2)",
    "components/lockup_tracker.py": "Lockup expiration tracker (Phase 2)",
    "components/financial_analysis.py": "Financial analysis view (Phase 2)",
    
    # Scrapers
    "scrapers/ipo_scraper.py": "IPO data pipeline from IPOScoop",
    "scrapers/sec_scraper.py": "SEC document scraper",
    
    # Data Files (Essential)
    "data/ipo_calendar.json": "Current IPO data",
    "data/watchlists.json": "User watchlists",
    "data/company_profiles.json": "Company information cache",
    
    # Configuration
    "config/ui_theme.json": "Terminal theme colors",
    "config/app_config.json": "Application settings"
}

# Generate the cleanup script separately
with open('cleanup_genesis_final.py', 'w', encoding='utf-8') as f:
    f.write('''"""
...
print("\n[GENERATED] cleanup_genesis_final.py")
print("\nTo execute cleanup:")
print("  python cleanup_genesis_final.py")
print("\nTo reverse if needed:")
print("  python reverse_genesis_cleanup.py")

# Show dependency map
print("\nDEPENDENCY MAP:")
print("-" * 80)
dependencies = {
    "app.py": ["services/*", "components/*", "config/ui_theme.json"],
    "services/ai_service.py": ["openai", ".env"],
    "services/data_service.py": ["data/*.json"],
    "services/sec_service.py": ["aiohttp", "beautifulsoup4"],
    "components/chat.py": ["services/ai_service.py"],
    "components/ipo_calendar.py": ["services/data_service.py", "scrapers/ipo_scraper.py"],
    "components/companies.py": ["services/sec_service.py", "services/data_service.py"],
    "components/financial_analysis.py": ["services/sec_service.py", "services/ai_service.py"],
    "scrapers/ipo_scraper.py": ["aiohttp", "beautifulsoup4"],
    "scrapers/sec_scraper.py": ["services/sec_service.py"]
}
...
print("\nFILES TO KEEP (Roadmap Required):")
print("-" * 80)
for file, desc in list(REQUIRED_FILES.items())[:10]:  # Show first 10
    print(f"  [KEEP] {file} - {desc}")
print("  ... and more")